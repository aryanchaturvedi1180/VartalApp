...
const HomePage = () => {
  ...
  const incomingReqCount = friendRequests?.incomingReqs?.length || 0;
  ...
  useEffect(() => {
    ...
    if (friends?.length > 0) {
      friends.forEach((friend) => friendsIdSet.add(friend._id));
    }

    setOutgoingRequestsIds(outgoingIds);
    setFriendsIds(friendsIdSet);
  }, [outgoingFriendReqs, friends]);
      ...
      <div className="container mx-auto space-y-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link to="/notifications" className="btn btn-outline btn-sm relative">
            ...
            Friend Requests
            {incomingReqCount > 0 && (
              <span className="badge badge-error text-white absolute -top-2 -right-2">
                {incomingReqCount}
              </span>
            )}
          </Link>
        ...
        <section>
          ...
          {loadingUsers ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommendedUsers
                .filter((user) => !friendsIds.has(user._id)) // Only hide if already friends
                .map((user) => {
